...
# Фильтруем данные для первого графика (только с 1800 секунд)
df1_filtered = df1[df1['Время'] >= 1950]

# Первый график: данные из файла возмущения (начиная с 1800 секунд)
plt.figure(figsize=(12, 6))
plt.plot(df1_filtered['Время'], df1_filtered['Вход1'], 'b-', label='Вход 1', linewidth=1)
plt.plot(df1_filtered['Время'], df1_filtered['Вход2'], 'r-', label='Вход 2', linewidth=1)
plt.xlabel('Время, с')
plt.ylabel('Значения')
...
plt.plot(df2['Время'], df2['Вход2'], 'r-', label='Вход 2', linewidth=1)
plt.xlabel('Время, с')
plt.ylabel('Значения')
...
print(f"Для первого графика отображено {len(df1_filtered)} строк (с 1800 секунд)")